...
2 principles that I followed are that it requires low physical effort to use. People are simply using a trackpad or mouse to select tabs or click buttons, and can type in information by clicking a button. It is also has appropriate size and space for approach and use, as the text size is a reasonable size to read, and, the button sizes are large enough to click, and the message/login information are also large enough to see. 2 principles that I may have violated are Tolerance for errors, as we were not required to handle all of the error return codes, so there may be some errors that arise. I also violated perceptible information, as there is only text on the screen. There are no images or verbal information, so the user can only read/type in information in order to use the applciation.
...
![Improvements for accessibility](_figures/accessibility.png)
